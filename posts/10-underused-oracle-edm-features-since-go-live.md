...
3. **The feature cadence itself is a signal.** A product shipping something EDM-specific almost every month versus one shipping three or four targeted governance fixes a year tells you where Oracle believes the remaining gaps actually are — and it isn't in core hierarchy management anymore. It's in governance at scale, which is exactly the stage most multi-year implementations are now in.
